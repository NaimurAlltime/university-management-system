...
export const CourseService = {
  ...
  async assignFaculties(courseId: string, faculties: string[]) {
    ...
    return Course.findByIdAndUpdate(courseId, { $set: { faculties: unique } }, { new: true }).populate("faculties")
  },

  async getFaculties(courseId: string) {
    return Course.findById(courseId).populate({
      path: "faculties",
      select: "name email designation academicDepartment academicFaculty",
      populate: [
        {
